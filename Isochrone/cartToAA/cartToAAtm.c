...
void cart_to_aa P(( double _tp1, double _tp2, double * _tp3, long _tpl3, double * _tp4, long _tpl4));
...
#endif
{
	...
	cart_to_aa(_tp1, _tp2, _tp3, _tpl3, _tp4, _tpl4);
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"CartToAA[Miso_Real, biso_Real, x_List, v_List]", (char *)"{Miso, biso, x, v}", 0);
