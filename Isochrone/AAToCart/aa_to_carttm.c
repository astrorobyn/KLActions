/*
 * This file automatically produced by /Software/users/math/Mathematica_8.0.1/SystemFiles/Links/MathLink/DeveloperKit//Linux-x86-64/CompilerAdditions/mprep from:
 *	aa_to_cart.tm
...
void aa_to_cart P(( double * _tp1, long _tpl1, double * _tp2, long _tpl2));
...
#endif
{
	...
	aa_to_cart(_tp1, _tpl1, _tp2, _tpl2);
...
static const char* evalstrs[] = {
	"AAToCart::usage = \"AAToCart[{Lz, L, Jr}, {th1, th2, th3}] takes ",
	"the three actions and their corresponding angles in the isochron",
	"e potential and calculates the corresponding Cartesian position ",
	"and velocity, returning a six-vector w={x,y,z,vx,vy,vz}. For pur",
	"ely radial orbits the inclination angle is undefined; these orbi",
	"ts are placed in the z=0 plane and a warning is printed. The pro",
	"gram returns a zero vector and prints an error message if the or",
	"bit is unbound or has Lz>L.\"",
	(const char*)0,
	(const char*)0
};
...
#endif
{
	int _res;
	_res = MLConnect(mlp);
	if (_res) _res = _definepattern(mlp, (char *)"AAToCart[J_List, th_List]", (char *)"{J, th}", 0);
